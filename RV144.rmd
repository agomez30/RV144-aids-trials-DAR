...
$H_0:$ There is no relationship between getting AIDS and having the AIDS vaccine.  
$H_a:$ There is a realtionship between getting AIDS and having the AIDS vaccine.  
```{r}
...
```
  ...
  The p-value of `r myrv144$p.value` result from the Chi Square test means the probability of getting a result like that just obtained if the null hypothesis is true is approximately 4.82 percent.  This is  normally considered significant.  
...
```{r}
myfish <- fisher.test(results)
myfish
```

  The p.value from the fisher test is `r myfish$p.value` and the confidence interval is `r myfish$conf.int`. There is evidence that the interval is not 1 but it is very close.  
   


## Conclusions

   With the data collected and the test results all being similar, we can conlclude that the null hypothesis is proved wrong.  There were less people who got the AIDS virus who recieved the AIDS vaccine than the people who only recieved the placebo.  We can conclude that the AIDS vaccine has made a significant impact on prevention of recieving the AIDS virus in people.
